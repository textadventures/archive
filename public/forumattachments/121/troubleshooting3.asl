...
define game <School Time>
	...
	verb <check> msg <You can't check that.>
	verb <search; look inside; look in; go through> msg <You can't search that.>
...
define room <Jerky Shop Interior>
	alias <jerky shop>
	prefix <inside a>
	look <This tiny shop brings to mind the aesthetic of an old west trading post.  The mingling scents of cedar and orange peel infuse the much stronger mouthwatering aroma of peppered meat, dried and hickory smoked.    The shop's offerings, a few varieties of homemade, packaged|b jerky|xb, appear to be in a locked display case.  The sales counter sits near the shop's entrance, upon which sits an old-fashioned |bcash register|xb.  On the far side of the counter, you see a small plastic plate labeled "samples.">
	indescription <You are:>
	east <Jerky Shop>

	define object <Sample Plate>
		alias <plate>
		look <This inexpensive white plate has the word "samples" written on it in permanent marker.>
		displaytype <Object>
		article <it>
		gender <it>
		invisible
	...
	define object <Display Jerky>
		alias <jerky>
		look <The case contains a variety of dried meat-  beef jerky, elk jerky, and a few others as well.  This jerky is for sale, but the display case appears to be locked, and nobody seems to be available to assist you.>
		prefix <packages of>
		suffix <from the display case>
		displaytype <Object>
		article <it>
		gender <it>
		invisible
	...
	define object <Sample Jerky>
		alias <jerky>
		alt <sample; jerky; elk jerky; jerky sample; sample of jerky; mesquite jerky; mesquite smoked jerky; sample of mesquite jerky; sample of elk jerky; elk jerky sample; samples; samples of jerky>
		look <This sample appears to be taken from the jerky shop's supply of mesquite smoked elk jerky.   Looks tasty.>
		take {
			give <Sample Jerky>
			msg <You reach across the counter and take the jerky.  In doing so, however, you make a startling discovery.  |n|nBehind the sales counter, a body lies motionless on the floor.>
			reveal <Body>
		}
		prefix <a free sample of>
		displaytype <Object>
		article <it>
		gender <it>
		parent <Sample Plate>
		action <eat> {
			if got <Sample Jerky> then {
				lose <Sample Jerky>
				msg <You munch on the jerky and find it to be quite awesome.  Yum.>
				show <Sample Jerky> } else msg <You don't have it.>
		}
	end define

	define object <Jumpsuit>
		alias <jumpsuit>
		alt <pockets; clothes; his pockets; the body's clothes; the body's pockets; clothing; his clothes; his clothing; his jumpsuit; pocket>
		look <The blue jumpsuit is quite generic- the sort worn by mechanics or industrial laborers.  There is a single pocket on the right front breast.  >
		displaytype <Object>
		article <it>
		gender <it>
		invisible
		properties <search>
		action <search> {
			if flag <body searched> then msg <There's nothing in the jumpsuit's pocket.> else {
				flag on <body searched>
				reveal <Nickel>
				msg <You hesitate to rifle through the pockets of a dead man, but your curiosity overcomes you.  Searching the jumpsuit, you find a single |bnickel|xb.>
				show <Nickel> }
		}
	end define
	...
	define object <Body>
		alias <body>
		alt <corpse; Peter; Peter's body; peter; peter's body; dead body>
		look <The body belongs to a middle-aged man, perhaps in his mid-fifties.  His facial features are strong, and his unkempt mane of salt-and-pepper hair matches his heavy eyebrows.  The body is dressed in a blue jumpsuit, and barefoot.  Above the jumpsuit's breast pocket is a patch that reads "PETER."  There are no visible signs of bleeding or trauma.|n|nHe's very obviously dead, although by the looks of him, whatever happened must have happened recently.>
		prefix <the>
		suffix <of a middle-aged white man>
	...
	define object <Nickel>
		alias <nickel>
		look <It's a regular five-cent piece.>
		take
		prefix <a>
		displaytype <Object>
		article <it>
	...
	define object <Jerky Register>
		alias <cash register>
		alt <register>
		look <This cash register is an all-mechanical holdover from simpler times.  It looks to be very sturdy.  >
		prefix <a>
		displaytype <Object>
		article <it>
		gender <it>
		invisible
		properties <open = It's locked shut.>
	end define
