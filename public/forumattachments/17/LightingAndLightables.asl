		...
		if not property <#this#; lit> then {
			if (#(this):with# = NULL) then { 
				...
				if type <#(this):with#; lighting> then {					
					if not got <#this#> and property <#this#; take> then {
...
define function <GetValidLightingObject>
	...
	for each object in <inventory> {
		if type <#quest.thing#; lighting> then {
			inc <i>
			property <game; foundObject_%i% = #quest.thing#>			
		}		
	}
	
	if (%i% > 0) then {
		for <n; 1; %i%> {
			set string <str; $objectproperty(game; foundObject_%n%)$>
			if (#(str):uses# > 0) then {				
				set <return_string; #str#>
			}
		}
	}
	...
	'cleanup
	for <n; 1; %i%> { 
		property <game; not foundObject_%i%> 
	}	
